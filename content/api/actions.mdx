...
<Callout type="warning">
  Runs on paid jobs only; a free job returns `JOB_TIER_REQUIRED`. Costs **one
  fit score**, charged to that job's own envelope. Requires the resume to be
...
Adds a note to the candidate's comment thread, visible to every collaborator
on the job in the dashboard. This is the plugin's flagship write: Claude's
assessment becomes a durable, team-visible note.
...
- **`text`**: required, plain text, 1 to 10,000 characters. Newlines become
  paragraphs, so the note renders naturally next to human comments.
...
To read the thread first, see
[list comments](/api/candidates#team-comments).
